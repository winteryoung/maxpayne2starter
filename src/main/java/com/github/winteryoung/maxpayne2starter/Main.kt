package com.github.winteryoung.maxpayne2starter
...
fun main(args: Array<String>) {
    ...
    println("Starting Max Payne 2...")

    if (File("MaxPayne2_cracked.exe").exists()) {
        ProcessBuilder("MaxPayne2_cracked.exe", if (args.size > 0) args[0] else "").start()
    } else {
        ProcessBuilder("MaxPayne2.exe", if (args.size > 0) args[0] else "").start()
    }

    println("Detecting Max Payne 2 game window...")
    val timer = Timer()
...
private fun procWindow(wnd: WinDef.HWND): String {
    ...
    if (text.toLowerCase().startsWith("Max Payne 2".toLowerCase())) {
